...
const Reservation = () => {
    ...
    // console.log('Thhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhh', log)
    ...
    return (
        <div className="w-full mt-14 md:mt-0 m-5 p-4 bg-white border border-gray-200 rounded-lg shadow sm:p-8 dark:bg-gray-800 dark:border-gray-700">
            <div className=" flex items-center justify-center mb-4 py-8">
                <h5 className="text-xl font-bold leading-none text-gray-900 dark:text-white ">Total Reservations</h5>
            </div>
            <div className="flow-root">
                <ul role="list" className="divide-y divide-gray-200 dark:divide-gray-700">
                    {
                        reserv.length > 0 ? (
                            reserv.map((singcard, i) => <div key={i}>
                            ...
                            <button onClick={() => alreadySeen(singcard._id)} className={singcard.condition === 'seen' ? 'btn btn-sm btn-ghost' : 'btn btn-sm btn-neutral'}>{singcard.condition}</button>
                                ...
                                <ResInfo name={singcard.name} persons={singcard.person} date={singcard.date} time={singcard.slot} phone={singcard.phone} email={singcard.email} />
                            </div>)) : (
                            <p className='text-2xl text-red-800 text-center '>"No Reservation "</p>
                        )
                    }
                </ul>
            </div>
